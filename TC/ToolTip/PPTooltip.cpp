...
//	EnableTextWrap(FALSE);
	SetDelayTime(PPTOOLTIP_TIME_INITIAL, 400);
...
BOOL CPPToolTip::RelayEvent(MSG* pMsg)
{
	...
	case WM_LBUTTONDOWN:
		...
		else if( IsVisible() && IsWindow(m_hWnd) )
		{
			Pop();
			POINT point = pMsg->pt;
			::ScreenToClient( m_hParentWnd, &point);
			::SendMessage(m_hParentWnd,pMsg->message,pMsg->wParam,pMsg->lParam);
			int pos = ::SendMessage(m_hParentWnd,SCI_POSITIONFROMPOINT,point.x,point.y); 
			::SendMessage(m_hParentWnd,SCI_SETSEL,pos,pos);
			return FALSE;
		}
