...
class OaAES : public CRijndael {

public:
	// the constructor - makes sure local things are initialized
	OaAES(void);

	int Encrypt(const unsigned char* inBuf, int inLen, unsigned char* outBuf, long *poutLen, __in const unsigned char* key);
	int Decrypt(const unsigned char* inBuf, long inLen, unsigned char* outBuf, long *poutLen, __in const unsigned char* key);
