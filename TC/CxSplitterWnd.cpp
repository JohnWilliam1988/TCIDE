...
// HitTest return values (values and spacing between values is important)// Had to adopt this because it has module scope 
enum HitTestValue
{
	noHit = 0,
	vSplitterBox = 1,
...
void CxSplitterWnd::StopTracking(BOOL bAccept)
{
	return CSplitterWnd::StopTracking(bAccept);
}
...
{
	// The code line below is necessary ifusing CxSplitterWnd in a regular dll// AFX_MANAGE_STATE(AfxGetStaticModuleState()); 
