...
CGridCtrl::CGridCtrl(int nRows, int nCols, int nFixedRows, int nFixedCols)
{
	...
	m_eClicked			= _T("");							//左键单击
	m_eBeforeEdit		= _T("");							//开始编辑
	m_eAfterEdit		= _T("");							//结束编辑
	m_eClickHeadSort	= _T("");							//点击排序

	m_bExpandToFit      = FALSE;							//自适应大小
	m_allCellsEditAbled = TRUE;								//所有项可编辑
...
void CGridCtrl::TCSetClickEvent(CString click)			{		m_eClicked = click;			}
void CGridCtrl::TCSetBeforeEditEvent(CString befor)		{		m_eBeforeEdit = befor;		}
void CGridCtrl::TCSetAfterEditEvent(CString after)		{		m_eAfterEdit = after;		}
void CGridCtrl::TCSetHeadSortEvent(CString headsort)	{	m_eClickHeadSort = headsort;	}
void CGridCtrl::TCSetAllSellsEditAbled(BOOL editAble)	{  m_allCellsEditAbled = editAble;	}

CString CGridCtrl::TCGetClickEvent()		{	return m_eClicked;			}
CString CGridCtrl::TCGetBeforeEditEvent()	{	return m_eBeforeEdit;		}
CString CGridCtrl::TCGetAfterEditEvent()	{	return m_eAfterEdit;		}
CString CGridCtrl::TCGetHeadSortEvent()		{	return m_eClickHeadSort;	}
...
BOOL  CGridCtrl::TCSetColReadInfo(int nRow, int nCol, BOOL reOnly)
{
	if (GetVirtualMode())
		return FALSE;
	...
	pCell->TCSetSellReadOnly(reOnly);
	SetModified(TRUE, nRow, nCol);
	return TRUE;
}

BOOL  CGridCtrl::TCGetColReadInfo(int nRow, int nCol)
{
	CGridCellBase* pCell = GetCell(nRow, nCol);
	if (!pCell)
		return (LPARAM) 0;

	return pCell->TCGetSellReadOnly();
}
...
bool CGridCtrl::IsShowVertiLines()
{
	switch(m_nGridLines)
	{
	case GVL_BOTH:
...
bool CGridCtrl::IsShowHorizLines()
{
	switch(m_nGridLines)
	{
	case GVL_BOTH:
