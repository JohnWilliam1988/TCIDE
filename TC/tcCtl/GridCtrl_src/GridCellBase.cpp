...
void CGridCellBase::operator=(const CGridCellBase& cell)
{
	...
	//////////////////重载运算符中记得添加自定义的字段//////////////////////////
	TCSetSellReadOnly(cell.TCGetSellReadOnly());
}
...
//TCGRIDCTRLTYPE  CGridCellBase::TCGetCellType() const
//{
//	return TCGRIDCTRLTYPE::TCGRID_CTRL_NOMAL;
//}
...
BOOL CGridCellBase::Draw(CDC* pDC, int nRow, int nCol, CRect rect,  BOOL bEraseBkgnd /*=TRUE*/)
{
    ...
    if (IsFixed() && pGrid->GetGridLines() != GVL_NONE)
    {
        ...
        {
            rect.right++; rect.bottom++;
            pDC->DrawEdge(rect, /*BDR_SUNKENINNER-tc修改选中时候的固顶项没有陷下去 2012-5-2*//*NULL*/ BDR_RAISEDOUTER, BF_RECT);
