...
class Aes  // Advanced Encryption Standard
{
 public:
	...
	void My_Encrypt(std::string stryuanwen,char *key,std::string &rstr);   //加密
	void My_Decrypt(std::string strciphertext,char *key,std::string &rstr);//解密
private:
	...
	void SubWord(unsigned char* word, unsigned char* temp);         //密钥S盒字代换
    void RotWord(unsigned char* word, unsigned char* temp);       //密钥移位
	int Aes::char2num(char ch);  //解密要用的到
};