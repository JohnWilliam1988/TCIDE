...
BOOL CXInfoTip::Create(CWnd* pParentWnd) 
{
    ...
    lstrcat(logFont.lfFaceName, _T("MS Shell Dlg 2"));
...
void CXInfoTip::OnPaint() 
{
	...
	WindowBrush.CreateSolidBrush(RGB(255,255,210)/*::GetSysColor(COLOR_WINDOW)*/);
	...
	dc.FillRect(&rc,&WindowBrush);
	dc.FrameRect(&rc,&InnerFrameBrush);
...
void CXInfoTip::OnTimer( UINT nIDEvent ) 
{
	...
	case timerShow:
		...
		SetWindowPos(&wndTop, m_ptOrigin.x/* - WindowSize.cx + CX_ROUNDED*/, m_ptOrigin.y /*- WindowSize.cy*/ + CY_ROUNDED, WindowSize.cx - CX_ROUNDED, WindowSize.cy - CY_ROUNDED - CY_LEADER, SWP_NOACTIVATE | SWP_SHOWWINDOW);
...
void CXInfoTip::RelayEvent(LPMSG lpMsg)
{
	...
	//case WM_LBUTTONDOWN:
		//ShowWindow(SW_HIDE);
		//GetParent()->SendMessage(WM_LBUTTONDOWN,lpMsg->wParam,lpMsg->lParam);
		//break;
	case WM_RBUTTONDOWN:
	case WM_MBUTTONDOWN:
		ShowWindow(SW_HIDE);
	...
	case WM_KEYDOWN:
		...
		if( pView != NULL )
		{
			int size = m_ToolMap.GetCount();
			if(m_ToolMap.Lookup(pView->m_hWnd, Info))
			{
				//Display the tooltip
				//列表选中行绘制区域
				CRect listRect;
				pView->m_listFaterView.m_listBox.GetWindowRect(&listRect);
				CRect focusRect = pView->m_listFaterView.m_listBox.m_focusItemRect;
				pView->m_listFaterView.m_listBox.ClientToScreen(&focusRect);
				point.x = listRect.right;
				point.y = focusRect.top - 12;
				m_ptOrigin = point;
				SetIcon(Info.hIcon);
				Show(Info.szText, &point);
			}
