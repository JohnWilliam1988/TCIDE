...
//  Includes
// ---------------------------------------------------------------------------
#include <match/parsers/DOMLSParserImpl.hpp>
...
#include <match/dom/impl/DOMErrorImpl.hpp>
#include <match/dom/impl/DOMLocatorImpl.hpp>
#include <match/dom/impl/DOMConfigurationImpl.hpp>
#include <match/dom/impl/DOMStringListImpl.hpp>
#include <match/dom/impl/DOMDocumentImpl.hpp>
#include <match/dom/DOMException.hpp>
#include <match/dom/DOMLSException.hpp>
#include <match/dom/DOMDocumentFragment.hpp>
#include <match/dom/DOMNamedNodeMap.hpp>
#include <match/internal/XMLScanner.hpp>
#include <match/framework/Wrapper4DOMLSInput.hpp>
...
#include <match/util/XMLDOMMsg.hpp>
...
, fWrapNodesContext(0)
{
    ...
    fSupportedParameters->add(XMLUni::fgMatchEntityResolver);
    fSupportedParameters->add(XMLUni::fgMatchSchemaExternalSchemaLocation);
	fSupportedParameters->add(XMLUni::fgMatchSchemaExternalNoNameSpaceSchemaLocation);
	fSupportedParameters->add(XMLUni::fgMatchSecurityManager);
	fSupportedParameters->add(XMLUni::fgMatchScannerName);
    fSupportedParameters->add(XMLUni::fgMatchParserUseDocumentFromImplementation);
    ...
    fSupportedParameters->add(XMLUni::fgMatchSchema);
    fSupportedParameters->add(XMLUni::fgMatchSchemaFullChecking);
    fSupportedParameters->add(XMLUni::fgMatchUserAdoptsDOMDocument);
    fSupportedParameters->add(XMLUni::fgMatchLoadExternalDTD);
    fSupportedParameters->add(XMLUni::fgMatchLoadSchema);
    fSupportedParameters->add(XMLUni::fgMatchContinueAfterFatalError);
    fSupportedParameters->add(XMLUni::fgMatchValidationErrorAsFatal);
    fSupportedParameters->add(XMLUni::fgMatchCacheGrammarFromParse);
    fSupportedParameters->add(XMLUni::fgMatchUseCachedGrammarInParse);
    fSupportedParameters->add(XMLUni::fgMatchCalculateSrcOfs);
    fSupportedParameters->add(XMLUni::fgMatchStandardUriConformant);
    fSupportedParameters->add(XMLUni::fgMatchDOMHasPSVIInfo);
    fSupportedParameters->add(XMLUni::fgMatchGenerateSyntheticAnnotations);
    fSupportedParameters->add(XMLUni::fgMatchValidateAnnotations);
    fSupportedParameters->add(XMLUni::fgMatchIdentityConstraintChecking);
    fSupportedParameters->add(XMLUni::fgMatchIgnoreCachedDTD);
    fSupportedParameters->add(XMLUni::fgMatchIgnoreAnnotations);
    fSupportedParameters->add(XMLUni::fgMatchDisableDefaultEntityResolution);
    fSupportedParameters->add(XMLUni::fgMatchSkipDTDValidation);
    fSupportedParameters->add(XMLUni::fgMatchDoXInclude);
    fSupportedParameters->add(XMLUni::fgMatchHandleMultipleImports);
...
void DOMLSParserImpl::setParameter(const XMLCh* name, const void* value)
{
    ...
    else if (XMLString::compareIStringASCII(name, XMLUni::fgMatchEntityResolver) == 0)
    {
    ...
    else if (XMLString::compareIStringASCII(name, XMLUni::fgMatchSchemaExternalSchemaLocation) == 0)
    {
      setExternalSchemaLocation((XMLCh*)value);
    }
    else if (XMLString::compareIStringASCII(name, XMLUni::fgMatchSchemaExternalNoNameSpaceSchemaLocation) == 0)
    {
      setExternalNoNamespaceSchemaLocation((XMLCh*)value);
    }
    else if (XMLString::compareIStringASCII(name, XMLUni::fgMatchSecurityManager) == 0)
    {
      setSecurityManager((SecurityManager*)value);
    }
    else if (XMLString::compareIStringASCII(name, XMLUni::fgMatchScannerName) == 0)
    {
        AbstractDOMParser::useScanner((const XMLCh*) value);
    }
    else if (XMLString::compareIStringASCII(name, XMLUni::fgMatchParserUseDocumentFromImplementation) == 0)
    {
        useImplementation((const XMLCh*) value);
    }
    else if (XMLString::compareIStringASCII(name, XMLUni::fgMatchLowWaterMark) == 0)
    {
...
void DOMLSParserImpl::setParameter(const XMLCh* name, bool state)
{
    ...
    else if (XMLString::compareIStringASCII(name, XMLUni::fgMatchSchema) == 0)
    {
        setDoSchema(state);
    }
    else if (XMLString::compareIStringASCII(name, XMLUni::fgMatchSchemaFullChecking) == 0)
    {
        setValidationSchemaFullChecking(state);
    }
    else if (XMLString::compareIStringASCII(name, XMLUni::fgMatchUserAdoptsDOMDocument) == 0)
    {
        if(state)
    ...
    else if (XMLString::compareIStringASCII(name, XMLUni::fgMatchLoadExternalDTD) == 0)
    {
        setLoadExternalDTD(state);
    }
    else if (XMLString::compareIStringASCII(name, XMLUni::fgMatchLoadSchema) == 0)
    {
        setLoadSchema(state);
    }
    else if (XMLString::compareIStringASCII(name, XMLUni::fgMatchContinueAfterFatalError) == 0)
    {
        setExitOnFirstFatalError(!state);
    }
    else if (XMLString::compareIStringASCII(name, XMLUni::fgMatchValidationErrorAsFatal) == 0)
    {
        setValidationConstraintFatal(state);
    }
    else if (XMLString::compareIStringASCII(name, XMLUni::fgMatchCacheGrammarFromParse) == 0)
    {
    ...
    else if (XMLString::compareIStringASCII(name, XMLUni::fgMatchUseCachedGrammarInParse) == 0)
    {
        if (state || !getScanner()->isCachingGrammarFromParse())
            getScanner()->useCachedGrammarInParse(state);
    }
    else if (XMLString::compareIStringASCII(name, XMLUni::fgMatchCalculateSrcOfs) == 0)
    {
        getScanner()->setCalculateSrcOfs(state);
    }
    else if (XMLString::compareIStringASCII(name, XMLUni::fgMatchStandardUriConformant) == 0)
    {
        getScanner()->setStandardUriConformant(state);
    }
    else if (XMLString::compareIStringASCII(name, XMLUni::fgMatchDOMHasPSVIInfo) == 0)
    {
        setCreateSchemaInfo(state);
    }
    else if (XMLString::compareIStringASCII(name, XMLUni::fgMatchGenerateSyntheticAnnotations) == 0)
    {
        getScanner()->setGenerateSyntheticAnnotations(state);
    }
    else if (XMLString::compareIStringASCII(name, XMLUni::fgMatchValidateAnnotations) == 0)
    {
        getScanner()->setValidateAnnotations(state);
    }
    else if (XMLString::compareIStringASCII(name, XMLUni::fgMatchIdentityConstraintChecking) == 0)
    {
        getScanner()->setIdentityConstraintChecking(state);
    }
    else if (XMLString::compareIStringASCII(name, XMLUni::fgMatchIgnoreCachedDTD) == 0)
    {
        getScanner()->setIgnoredCachedDTD(state);
    }
    else if (XMLString::compareIStringASCII(name, XMLUni::fgMatchIgnoreAnnotations) == 0)
    {
        getScanner()->setIgnoreAnnotations(state);
    }
    else if (XMLString::compareIStringASCII(name, XMLUni::fgMatchDisableDefaultEntityResolution) == 0)
    {
        getScanner()->setDisableDefaultEntityResolution(state);
    }
    else if (XMLString::compareIStringASCII(name, XMLUni::fgMatchSkipDTDValidation) == 0)
    {
        getScanner()->setSkipDTDValidation(state);
    }
    else if (XMLString::compareIStringASCII(name, XMLUni::fgMatchDoXInclude) == 0)
    {
        setDoXInclude(state);
    }
    else if (XMLString::compareIStringASCII(name, XMLUni::fgMatchHandleMultipleImports) == 0)
    {
...
bool DOMLSParserImpl::canSetParameter(const XMLCh* name, const void* /*value*/) const
{
    if (XMLString::compareIStringASCII(name, XMLUni::fgDOMResourceResolver) == 0 ||
        ...
        XMLString::compareIStringASCII(name, XMLUni::fgMatchEntityResolver) == 0 ||
        XMLString::compareIStringASCII(name, XMLUni::fgMatchSchemaExternalSchemaLocation) == 0 ||
        XMLString::compareIStringASCII(name, XMLUni::fgMatchSchemaExternalNoNameSpaceSchemaLocation) == 0 ||
        XMLString::compareIStringASCII(name, XMLUni::fgMatchSecurityManager) == 0 ||
        XMLString::compareIStringASCII(name, XMLUni::fgMatchScannerName) == 0 ||
        XMLString::compareIStringASCII(name, XMLUni::fgMatchParserUseDocumentFromImplementation) == 0 ||
        XMLString::compareIStringASCII(name, XMLUni::fgMatchLowWaterMark) == 0)
      return true;
    else if(XMLString::compareIStringASCII(name, XMLUni::fgDOMSchemaLocation) == 0 ||
...
bool DOMLSParserImpl::canSetParameter(const XMLCh* name, bool value) const
{
    if (XMLString::compareIStringASCII(name, XMLUni::fgDOMCharsetOverridesXMLEncoding) == 0 ||
        ...
        XMLString::compareIStringASCII(name, XMLUni::fgMatchSchema) == 0 ||
        XMLString::compareIStringASCII(name, XMLUni::fgMatchSchemaFullChecking) == 0 ||
        XMLString::compareIStringASCII(name, XMLUni::fgMatchIdentityConstraintChecking) == 0 ||
        XMLString::compareIStringASCII(name, XMLUni::fgMatchLoadExternalDTD) == 0 ||
        XMLString::compareIStringASCII(name, XMLUni::fgMatchLoadSchema) == 0 ||
        XMLString::compareIStringASCII(name, XMLUni::fgMatchContinueAfterFatalError) == 0 ||
        XMLString::compareIStringASCII(name, XMLUni::fgMatchValidationErrorAsFatal) == 0 ||
        XMLString::compareIStringASCII(name, XMLUni::fgMatchCacheGrammarFromParse) == 0 ||
        XMLString::compareIStringASCII(name, XMLUni::fgMatchUseCachedGrammarInParse) == 0 ||
        XMLString::compareIStringASCII(name, XMLUni::fgMatchCalculateSrcOfs) == 0 ||
        XMLString::compareIStringASCII(name, XMLUni::fgMatchStandardUriConformant) == 0 ||
        XMLString::compareIStringASCII(name, XMLUni::fgMatchUserAdoptsDOMDocument) == 0 ||
        XMLString::compareIStringASCII(name, XMLUni::fgMatchDOMHasPSVIInfo) == 0 ||
        XMLString::compareIStringASCII(name, XMLUni::fgMatchGenerateSyntheticAnnotations) == 0 ||
        XMLString::compareIStringASCII(name, XMLUni::fgMatchValidateAnnotations) == 0 ||
        XMLString::compareIStringASCII(name, XMLUni::fgMatchIgnoreCachedDTD) == 0 ||
        XMLString::compareIStringASCII(name, XMLUni::fgMatchIgnoreAnnotations) == 0 ||
        XMLString::compareIStringASCII(name, XMLUni::fgMatchDisableDefaultEntityResolution) == 0 ||
        XMLString::compareIStringASCII(name, XMLUni::fgMatchSkipDTDValidation) == 0 ||
		XMLString::compareIStringASCII(name, XMLUni::fgMatchDoXInclude) == 0 ||
        XMLString::compareIStringASCII(name, XMLUni::fgMatchHandleMultipleImports) == 0)
      return true;
    else if(XMLString::compareIStringASCII(name, XMLUni::fgDOMDisallowDoctype) == 0 ||
...
DOMDocument* DOMLSParserImpl::parse(const DOMLSInput* source)
{
    ...
    // Disabled until 4.0.0. See MATCHC-1894 for details.
    //if(getErrorCount()!=0)
