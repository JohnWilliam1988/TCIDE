...
//文件建立修改记录
//索引      日期        记录人      动作以及目的 
//0001      2010-06-26  肖秀范      完成文件读取类MyReader的设计,在原计划上添加一个 bool getLine(XMLBuffer& toFill);方法
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

#if !defined(MATCHC_INCLUDE_GUARD_MYREADER_HPP)
#define MATCHC_INCLUDE_GUARD_MYREADER_HPP

#include <match/util/XMLChar.hpp>
#include <match/framework/XMLRecognizer.hpp>
#include <match/framework/XMLBuffer.hpp>
#include <match/util/TranscodingException.hpp>
...
//
// ---------------------------------------------------------------------------
class XMLPARSER_EXPORT MyReader : public XMemory
{
public:
    // -----------------------------------------------------------------------
    //  Public types
    ...
    bool getLine(XMLBuffer& toFill);                    //自定义一个方法,用来读取一行数据 - match 6-30 
...
private:
    ...
    // -----------------------------------------------------------------------
    MyReader(const MyReader&);
    MyReader& operator=(const MyReader&);
...
// ---------------------------------------------------------------------------
//  MyReader: Public, query methods
// ---------------------------------------------------------------------------
inline bool MyReader::isNameChar(const XMLCh toCheck) const
{
    return ((fgCharCharsTable[toCheck] & gNameCharMask) != 0);
}

inline bool MyReader::isNCNameChar(const XMLCh toCheck) const
{
    return ((fgCharCharsTable[toCheck] & gNCNameCharMask) != 0);
}

inline bool MyReader::isPlainContentChar(const XMLCh toCheck) const
{
    return ((fgCharCharsTable[toCheck] & gPlainContentCharMask) != 0);
}


inline bool MyReader::isFirstNameChar(const XMLCh toCheck) const
{
    return ((fgCharCharsTable[toCheck] & gFirstNameCharMask) != 0);
}

inline bool MyReader::isFirstNCNameChar(const XMLCh toCheck) const
{
    return (((fgCharCharsTable[toCheck] & gFirstNameCharMask) != 0)
            && (toCheck != chColon));
}

inline bool MyReader::isSpecialStartTagChar(const XMLCh toCheck) const
{
    return ((fgCharCharsTable[toCheck] & gSpecialStartTagCharMask) != 0);
}

inline bool MyReader::isXMLChar(const XMLCh toCheck) const
{
    return ((fgCharCharsTable[toCheck] & gXMLCharMask) != 0);
}

inline bool MyReader::isXMLLetter(const XMLCh toCheck) const
{
    return (((fgCharCharsTable[toCheck] & gFirstNameCharMask) != 0)
            && (toCheck != chColon) && (toCheck != chUnderscore));
}

inline bool MyReader::isWhitespace(const XMLCh toCheck) const
{
    return ((fgCharCharsTable[toCheck] & gWhitespaceCharMask) != 0);
}

inline bool MyReader::isControlChar(const XMLCh toCheck) const
{
    return ((fgCharCharsTable[toCheck] & gControlCharMask) != 0);
}

// ---------------------------------------------------------------------------
//  MyReader: Buffer management methods
// ---------------------------------------------------------------------------
inline XMLSize_t MyReader::charsLeftInBuffer() const
{
...
// ---------------------------------------------------------------------------
//  MyReader: Getter methods
// ---------------------------------------------------------------------------
inline XMLFileLoc MyReader::getColumnNumber() const
{
    return fCurCol;
}

inline const XMLCh* MyReader::getEncodingStr() const
{
    return fEncodingStr;
}

inline XMLFileLoc MyReader::getLineNumber() const
{
    return fCurLine;
}

inline bool MyReader::getNoMoreFlag() const
{
    return fNoMore;
}

inline const XMLCh* MyReader::getPublicId() const
{
    return fPublicId;
}

inline XMLSize_t MyReader::getReaderNum() const
{
    return fReaderNum;
}

inline MyReader::RefFrom MyReader::getRefFrom() const
{
    return fRefFrom;
}

inline MyReader::Sources MyReader::getSource() const
{
    return fSource;
}

inline const XMLCh* MyReader::getSystemId() const
{
    return fSystemId;
}

inline bool MyReader::getThrowAtEnd() const
{
    return fThrowAtEnd;
}

inline MyReader::Types MyReader::getType() const
{
    return fType;
}

// ---------------------------------------------------------------------------
//  MyReader: Setter methods
// ---------------------------------------------------------------------------
inline void MyReader::setReaderNum(const XMLSize_t newNum)
{
    fReaderNum = newNum;
}

inline void MyReader::setThrowAtEnd(const bool newValue)
{
    fThrowAtEnd = newValue;
}

inline void MyReader::setXMLVersion(const XMLVersion version)
{
    fXMLVersion = version;
    if (version == XMLV1_1) {
        fNEL = true;
...
// ---------------------------------------------------------------------------
//
//  MyReader: movePlainContentChars()
//
...
//
// ---------------------------------------------------------------------------
inline void MyReader::movePlainContentChars(XMLBuffer &dest)
{
...
// ---------------------------------------------------------------------------
//  MyReader: getNextCharIfNot() method inlined for speed
// ---------------------------------------------------------------------------
inline bool MyReader::getNextCharIfNot(const XMLCh chNotToGet, XMLCh& chGotten)
{
    //
...
// ---------------------------------------------------------------------------
//  MyReader: getNextChar() method inlined for speed
// ---------------------------------------------------------------------------
inline bool MyReader::getNextChar(XMLCh& chGotten)
{
    //
    //  See if there is at least a char in the buffer. Else, do the buffer
    //  reload logic.
...
// ---------------------------------------------------------------------------
//  MyReader: peekNextChar() method inlined for speed
// ---------------------------------------------------------------------------
inline bool MyReader::peekNextChar(XMLCh& chGotten)
{
    //
