...
#if !defined(MATCHC_INCLUDE_GUARD_PLATFORMUTILS_HPP)
#define MATCHC_INCLUDE_GUARD_PLATFORMUTILS_HPP

#include <match/util/MatchDefs.hpp>
#include <match/util/XMLException.hpp>
#include <match/util/PanicHandler.hpp>
...
public :
    ...
    /** Perform per-process parser initialization
      ...
      */
    static void Initialize(const char*          const locale = XMLUni::fgMatchcDefaultLocale
    ...
    static void Initialize(XMLSize_t initialDOMHeapAllocSize
                         ...
                         , const char*          const locale = XMLUni::fgMatchcDefaultLocale
                         , const char*          const nlsHome = 0
                         ,       PanicHandler*  const panicHandler = 0
                         ,       MemoryManager* const memoryManager = 0);
    ...
    /** Unlocks a mutex
      ...
      * are used, Match cannot guarantee that lock/unlock operations are
...
private :
    ...
    /**
      ...
      * Match uses a default one that it owns and is responsible for
