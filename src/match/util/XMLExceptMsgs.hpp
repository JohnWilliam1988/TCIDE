...
#if !defined(MATCHC_INCLUDE_GUARD_ERRHEADER_XMLExcepts)
#define MATCHC_INCLUDE_GUARD_ERRHEADER_XMLExcepts
...
public :
    enum Codes
    {
      ...
      , E_HighBounds                       = 368
		...
		///////////////////////////////////////////自定义异常/////////////////////////////////////
		//效验中的异常
		, my_validator_VarNameErr				= 480	// {0}是不合法的变量名或者是没定义的变量名
		, my_validator_ParentBlockErr        	= 481	// 错误的父块关系
		, my_validator_BlockInheritErr       	= 482	// 错误的块继承关系
		, my_validator_FuncNameErr           	= 483	// {0}是不正确的函数名
		, my_validator_NoPairBracket         	= 484	// {0}是错误的函数参数列表,没有找到成对的括号括起来
		, my_validator_DefaultParamErr         	= 485	// {0}函数定义的参数列表中,只要有一个默认参数,后面的参数都必须是默认参数
		, my_validator_PramNameErr           	= 486	// {0}是不正确的参数名
		, my_validator_NoFindLeftBracket     	= 487	// {0}没有找到参数表达的左括号"("
		, my_validator_UnDefFuncName         	= 488	// {0}是未定义的函数
		, my_validator_PramListErr           	= 489	// {0}是错误的参数列表     此异常未用
		, my_validator_ArgumentListErr       	= 490	// ({0})是错误的实参数列表
		, my_validator_PramAndArgumDiscordance 	= 491	// 实参数列表{0}与形参列表{1}不对应
		, my_validator_StrConstErr           	= 492	// {0}是错误的字符型常量
		, my_validator_ConstErr              	= 493	// {0}是不正确的常量
		, my_validator_ExpresstionErr        	= 494	// {0}是错误的表达式
		, my_validator_OperationPramErr      	= 495	// 运算中参数出现错误的类型或者为不支持的数据类型
		, my_validator_OperationLeftPramErr  	= 496	// 运算中左参数出现错误的类型
		, my_validator_OperationRightPramErr 	= 497	// 运算中右参数出现错误的类型
		, my_validator_LogicOperationPramErr 	= 498	// 逻辑运算中参数必须为布尔类型(bool)
		, my_validator_CompareOperationPramErr 	= 499	// 比较运算中参数必须为数值类型
		, my_validator_CallFuncNameFindSpace 	= 500	// {0}函数调用中包含多余的空格
		, my_validator_CallFuncErr           	= 501	// {0} 为函数调用错误或者未定义函数未定义的使用
		, my_validator_ReturnTypeErr         	= 502	// return语句的返回类型与函数声明的返回类型不一致
		, my_validator_NamedVarErr           	= 503	// {0}是不正确的命名定义,只有以下划线和字母开头进行命名,长度不能超过32
		, my_validator_NamedMidFindUnstrErr  	= 504	// {0}中出现不是命名规范中的字符
		, my_validator_NameSimilarKeyword    	= 505	// {0}是关键字，不能用于空间命名、函数名或变量名的定义
		, my_validator_NoFindMain            	= 506	// 找不到main函数
		, my_validator_ErroEscapeType          	= 507	// {0}中出现不正确的转义字符格式
        , my_validator_StatementTooLong        	= 508	// 语句过长,长度不永许超过1024个长度

		//match效验块的异常
		,my_match_AddBlockErr                   = 510   // {0},添加块节点时,出现头节点或者尾节点为空
		,my_match_BlockTypeErr                  = 511   // {0} 块类型错误,不被允许执行
		,my_match_FilePathIsEmpty               = 512   // {0},路径不能为空
		,my_match_NameIsEmpty                   = 513   // {0} 不能为空
		,my_match_EndStatmentMore               = 514   // {0} 中结束语句过多
		,my_match_ResultIsnotBool               = 515   // {0} 中的表达式 {1} 结果不是Bool型,应该是Bool型
		,my_match_IncludeStatmentErr            = 516   // Include中的字符串:{0} 格式不正确
		,my_match_AssignmentFail                = 517   // 赋值语句的左右两边数据类型不一致
		,my_match_VarDefineTypeErr              = 518   // 变量{0}定义的数据类型不支持,是错误的数据类型
		//数组相关
		,my_array_DefOverCount                  = 519   // 数组定义'{0}',超过了目前数组定义的最大维数,最大维数3维
		,my_array_CallOverCount                 = 520   // 数组调用'{0}',超过了目前数组的最大维数,最大维数3维
		// match执行中的异常
		,my_matchParser_VarIsExist              = 550   // 变量不存在
		,my_matchParser_RunExpressFail          = 551   // {0}计算失败
		,my_matchParser_AssignmentFail          = 552   // {0}中，赋值不成功


		//APIXML 解析异常
		,my_XMLAnalyze_APIDllNameIsNULL         = 600   // XML中DLL名不能为空
		,my_XMLAnalyze_APINameSPNameIsNULL      = 601   // XML中命名空间名不能为空
		,my_XMLAnalyze_APIFuncNameIsNULL        = 602   // XML中API函数名不能为空
		,my_XMLAnalyze_APIPathIsNULL            = 603   // XML中API路径不能为空
		,my_XMLAnalyze_APIDllListInserErr       = 604   // XML中APIDll链表添加Dll节点时出现头节点或者尾节点为空
		,my_XMLAnalyze_APINameSPListInserErr    = 605   // XML中API命名空间链表添加命名空间节点时出现头节点或者尾节点为空
		,my_XMLAnalyze_APIFuncListInserErr      = 606   // XML中API函数链表添加函数节点时出现头节点或者尾节点为空
		,my_XMLAnalyze_APIPathError             = 607   // APIXML的路径不正确，请检查此路径:'{0}'

		// API
		,my_validator_IsVipFun                   = 608   //'{0}.{1}'是VIP功能,请登录VIP后再使用
