...
#if !defined(MATCHC_INCLUDE_GUARD_XMLSTRING_HPP)
#define MATCHC_INCLUDE_GUARD_XMLSTRING_HPP

#include <match/util/BaseRefVectorOf.hpp>
...
////////////////////////////////////////////////自定义方法-match////////////////////////////////////
	//得到缓冲区的第一个字符串
	static int getFirstString(XMLCh *pBuff,XMLCh* const outStr,int maxLength);

    //去掉两边的字符串的引号,注意只去掉第一层
	static void trimDoubleQuote(XMLCh* const toTrim);

	//TC专用字符查找方法,主要是用来参数列表里面查找,逗号,直接跳过字符串用的.
	static int TCIndexof(const XMLCh* const toSearch, const XMLCh ch);

    //去掉两边的括号,注意只去掉第一层
    static void trimParen(XMLCh* const toTrim);
        /**
    ...
    //判断是不是中文字符
    static bool isChinese(XMLCh toCheck);
    //判断是不是中文字符

    //判断是不是转义字符
    static XMLCh escapeCharacter(const XMLCh* toCheck);
        //判断是不是转义字符
//////////////////////// ///////////////////////自定义方法///////////////////////////////////////////
private :
