...
#include <match/util/ArrayIndexOutOfBoundsException.hpp>
#include <match/util/IllegalArgumentException.hpp>
...
#include <match/util/XMLURL.hpp>
#include <match/internal/XMLReader.hpp>
#include "match/my/MyValidator.h"
...
//得到缓冲区的第一个字符串
int XMLString::getFirstString(XMLCh *pBuff,XMLCh* const outStr,int maxLength)
{
	...
	outStr[maxLength-4]=L'.';
	outStr[maxLength-3]=L'.';
	outStr[maxLength-2]=L'.';
	outStr[maxLength-1]=0;
	return maxLength-1;
}

//去掉两边的字符串的引号,注意只去掉第一层
void XMLString::trimDoubleQuote(XMLCh* const toTrim)
{
    ...
    for (scrape = len; scrape > skip; scrape--)
    {
        if (chDoubleQuote!=toTrim[scrape - 1])
            break;
		else
		{
			scrape--;
...
int XMLString::TCIndexof(const XMLCh* const toSearch, const XMLCh ch)
{
    if (!toSearch || !*toSearch) return -1;
	//MyValidator::writelog(L"字符串操作开始------->");
	//XMLCh logmsg[256]={0};
	//::wsprintf(logmsg,L"%s %c",toSearch,ch);
	//	MyValidator::writelog(logmsg);
    ...
    while (*srcPtr)
	{
		if ((*srcPtr)==L'\"')
		{
			...
			while((*srcPtr)!=L'\"')
			{	
				//2010-9-18 加入转义字符的处理.--------钟斌---------
				if((*srcPtr)==L'\\')
				{
					//srcPtr +=2;
					ptr=XMLString::escapeCharacter(srcPtr);
					if( ptr!=0)
					{
						srcPtr +=2;
					}
					...
					{
						//"\"不能单独出来，只有做转义字符的时候才能出现，否则要出现需是"\\"
						//'\'的后面只能跟制定的转义字符，其他全报错                 --------钟斌
						//ThrowXMLwithMemMgr1(MyValidException,XMLExcepts::my_validator_ErroEscapeType,toSearch,XMLPlatformUtils::fgMemoryManager);
						return -1;
					}
				}
				else
		...
		else if((*srcPtr)==L'(')
		{
			...
			while ((*srcPtr)) 
			{
				...
				//这里是吃掉函数调用,参数列表里面的字符串常量 match 12-6
				else if((*srcPtr)==L'\"')
				{
					srcPtr++;
					while((*srcPtr)!=L'\"') 
						...
						if((*srcPtr))
						{
							...
							if((*srcPtr)==L'\\')
							{
								...
								else
								{
									//"\"不能单独出来，只有做转义字符的时候才能出现，否则要出现需是"\\"
									//'\'的后面只能跟制定的转义字符，其他全报错                 --------钟斌
									//ThrowXMLwithMemMgr1(MyValidException,XMLExcepts::my_validator_ErroEscapeType,fLastStr,XMLPlatformUtils::fgMemoryManager);
									return -1;
								}
							}
							else
		...
		if(!(*srcPtr))
		{
			//MyValidator::writelog(L"字符串操作结束------->");
			return -1;
		}
        if (ch == *srcPtr++)
		{
...
//去掉两边的字符串的括号,注意只去掉第一层
void XMLString::trimParen(XMLCh* const toTrim)
{
    const unsigned int len = stringLen(toTrim);
    ...
    for (scrape = len; scrape > skip; scrape--)
    {
        if (chCloseParen!=toTrim[scrape - 1])
            break;
		else
		{
			scrape--;
...
bool XMLString::isChinese(XMLCh toCheck)
{
    return ((0x4e00<=toCheck)&&(toCheck<=0x9fa5));
}



//判断是不是转义字符
XMLCh XMLString::escapeCharacter(const XMLCh* toCheck)
{
    const XMLCh* ptrStr=NULL;
    ...
    else if(*(ptrStr+1)==L'r')
    {
        return L'\r';
    }
    else if(*(ptrStr+1)==L't')
    {
        return L'\t';
    }
    else 
