...
#	include <sys/timeb.h>
#endif
#if HAVE_CPUID_H && !MATCH_HAVE_INTRIN_H
#   include <cpuid.h>
#endif
...
#include <match/util/DefaultPanicHandler.hpp>
#include <match/util/XMLInitializer.hpp>
#include <match/internal/MemoryManagerImpl.hpp>

#if MATCH_HAVE_INTRIN_H
#   include <intrin.h>
#endif

#include <match/util/XMLFileMgr.hpp>
#if MATCH_USE_FILEMGR_POSIX
#	include <match/util/FileManagers/PosixFileMgr.hpp>
#endif
#if MATCH_USE_FILEMGR_WINDOWS
#	include <match/util/FileManagers/WindowsFileMgr.hpp>
#endif

#include <match/util/XMLMutexMgr.hpp>
#if MATCH_USE_MUTEXMGR_NOTHREAD
#	include <match/util/MutexManagers/NoThreadMutexMgr.hpp>
#endif
#if MATCH_USE_MUTEXMGR_POSIX
#	include <match/util/MutexManagers/PosixMutexMgr.hpp>
#endif
#if MATCH_USE_MUTEXMGR_WINDOWS
#	include <match/util/MutexManagers/WindowsMutexMgr.hpp>
#endif

#include <match/util/XMLNetAccessor.hpp>
#if MATCH_USE_NETACCESSOR_CURL
#	include <match/util/NetAccessors/Curl/CurlNetAccessor.hpp>
#endif
#if MATCH_USE_NETACCESSOR_SOCKET
#	include <match/util/NetAccessors/Socket/SocketNetAccessor.hpp>
#endif
#if MATCH_USE_NETACCESSOR_CFURL
#	include <match/util/NetAccessors/MacOSURLAccessCF/MacOSURLAccessCF.hpp>
#endif
#if MATCH_USE_NETACCESSOR_WINSOCK
#	include <match/util/NetAccessors/WinSock/WinSockNetAccessor.hpp>
#endif


#include <match/util/XMLMsgLoader.hpp>
#if MATCH_USE_MSGLOADER_ICU
#	include <match/util/MsgLoaders/ICU/ICUMsgLoader.hpp>
#endif
#if MATCH_USE_MSGLOADER_ICONV
#	include <match/util/MsgLoaders/MsgCatalog/MsgCatalogLoader.hpp>
#endif
#if MATCH_USE_MSGLOADER_INMEMORY
#	include <match/util/MsgLoaders/InMemory/InMemMsgLoader.hpp>
#endif
#if MATCH_USE_WIN32_MSGLOADER
#	include <match/util/MsgLoaders/Win32/Win32MsgLoader.hpp>
#endif

#include <match/util/TransService.hpp>
#if MATCH_USE_TRANSCODER_ICU
#	include <match/util/Transcoders/ICU/ICUTransService.hpp>
#endif
#if MATCH_USE_TRANSCODER_GNUICONV
#	include <match/util/Transcoders/IconvGNU/IconvGNUTransService.hpp>
#endif
#if MATCH_USE_TRANSCODER_ICONV
#	include <match/util/Transcoders/Iconv/IconvTransService.hpp>
#endif
#if MATCH_USE_TRANSCODER_MACOSUNICODECONVERTER
#	include <match/util/Transcoders/MacOSUnicodeConverter/MacOSUnicodeConverter.hpp>
#endif
#if MATCH_USE_TRANSCODER_WINDOWS
#	include <match/util/Transcoders/Win32/Win32TransService.hpp>
#endif
...
#if defined (MATCH_USE_NETACCESSOR_CURL)
		na = new CurlNetAccessor();
#elif defined (MATCH_USE_NETACCESSOR_SOCKET)
		na = new SocketNetAccessor();
#elif defined (MATCH_USE_NETACCESSOR_CFURL)
		na = new MacOSURLAccessCF();
#elif defined (MATCH_USE_NETACCESSOR_WINSOCK)
...
XMLMsgLoader* XMLPlatformUtils::loadAMsgSet(const XMLCh* const msgDomain)
{
    ...
    try
    {
	#if defined (MATCH_USE_MSGLOADER_ICU)
		ms = new ICUMsgLoader(msgDomain);
	#elif defined (MATCH_USE_MSGLOADER_ICONV)
		ms = new MsgCatalogLoader(msgDomain);
    #elif defined (MATCH_USE_WIN32_MSGLOADER)
		ms = new Win32MsgLoader(msgDomain);
	#elif defined (MATCH_USE_MSGLOADER_INMEMORY)
...
XMLTransService* XMLPlatformUtils::makeTransService()
{
	...
	#if defined   (MATCH_USE_TRANSCODER_ICU)
		tc = new ICUTransService(fgMemoryManager);
	#elif defined (MATCH_USE_TRANSCODER_GNUICONV)
		tc = new IconvGNUTransService(fgMemoryManager);
	#elif defined (MATCH_USE_TRANSCODER_ICONV)
		tc = new IconvTransService(fgMemoryManager);
	#elif defined (MATCH_USE_TRANSCODER_MACOSUNICODECONVERTER)
		tc = new MacOSUnicodeConverter(fgMemoryManager);
	#elif defined (MATCH_USE_TRANSCODER_WINDOWS)
...
XMLPlatformUtils::makeFileMgr(MemoryManager* const memmgr)
{
	...
	#elif MATCH_USE_FILEMGR_WINDOWS
...
{
	// As far as we know, all supported Match
	// platforms use at least a forward slash
	// as a path delimiter. So we always check for
	// that.
	//
	// If MATCH_PATH_DELIMITER_BACKSLASH evaluates to true,
	// we also consider that as a slash.
	//
	// MATCH_PATH_DELIMITER_BACKSLASH may be set in config.h
	// by configure, or elsewhere by platform-specific
	// code.
    return	(
			false
		 || chForwardSlash == c
	#if MATCH_PATH_DELIMITER_BACKSLASH
		 || chBackSlash == c
	#endif
     		);
}
...
XMLMutexMgr* XMLPlatformUtils::makeMutexMgr(MemoryManager* const memmgr)
{
	...
	#elif MATCH_USE_MUTEXMGR_POSIX
		mgr = new (memmgr) PosixMutexMgr;
	#elif MATCH_USE_MUTEXMGR_WINDOWS
