...
#include <match/validators/common/Grammar.hpp>
 ...
 ***/
#ifdef MATCH_DEBUG_SORT_GRAMMAR
...
void XTemplateSerializer::storeObject(RefVectorOf<MatchLocationPath>* const objToStore
                                    , XSerializeEngine&                      serEng)
{
    ...
    if (serEng.needToStoreObject(objToStore))
    {
        ...
        for ( XMLSize_t i = 0; i < vectorLength; i++)
        {
            MatchLocationPath* data = objToStore->elementAt(i);
            serEng<<data;
        }
    }
...
void XTemplateSerializer::loadObject(RefVectorOf<MatchLocationPath>** objToLoad
                                   , int                               initSize
                                   , bool                              toAdopt
                                   , XSerializeEngine&                 serEng)
{
...
void XTemplateSerializer::storeObject(RefVectorOf<MatchStep>* const objToStore
                                    , XSerializeEngine&              serEng)
{
    ...
    if (serEng.needToStoreObject(objToStore))
    {
        ...
        for (XMLSize_t i = 0; i < vectorLength; i++)
        {
            MatchStep* data = objToStore->elementAt(i);
            serEng<<data;
        }
    }
...
void XTemplateSerializer::loadObject(RefVectorOf<MatchStep>** objToLoad
                                   , int                       initSize
                                   , bool                      toAdopt
                                   , XSerializeEngine&         serEng)
{
...
void XTemplateSerializer::storeObject(RefHashTableOf<MatchGroupInfo>* const objToStore
                                    , XSerializeEngine&                      serEng)
{
    if (serEng.needToStoreObject(objToStore))
    {
        ...
        RefHashTableOfEnumerator<MatchGroupInfo> e(objToStore, false, objToStore->getMemoryManager());
        XMLSize_t itemNumber = 0;
        ...
        for (XMLSize_t i=0; i < itemNumber; i++)
        {
            ...
            MatchGroupInfo* data = objToStore->get(strKey);
            serEng<<data;
        }
#else
        while (e.hasMoreElements())
        {
           ...
           // key = StringPool->getValueForId(MatchGroupInfo::getNameSpaceId())
           //     + chComma
           //     + StringPool->getValueForId(MatchGroupInfo::getNameId())
           //
           ...
           //
            ...
            MatchGroupInfo* data = objToStore->get(key);
            serEng<<data;
        }
#endif
    }
...
void XTemplateSerializer::loadObject(RefHashTableOf<MatchGroupInfo>** objToLoad
                                   , int
                                   , bool                              toAdopt
                                   , XSerializeEngine&                 serEng)
{
...
void XTemplateSerializer::storeObject(RefHashTableOf<MatchAttGroupInfo>* const objToStore
                                    , XSerializeEngine&                         serEng)
{
    ...
    if (serEng.needToStoreObject(objToStore))
    {
        ...
        RefHashTableOfEnumerator<MatchAttGroupInfo> e(objToStore, false, objToStore->getMemoryManager());
        XMLSize_t itemNumber = 0;
        ...
        for (XMLSize_t i=0; i < itemNumber; i++)
        {
            ...
            MatchAttGroupInfo* data = objToStore->get(strKey);
            serEng<<data;
        }
#else
        while (e.hasMoreElements())
        {
            MatchAttGroupInfo* data = objToStore->get(e.nextElementKey());
            serEng<<data;
        }
#endif
    }
...
void XTemplateSerializer::loadObject(RefHashTableOf<MatchAttGroupInfo>** objToLoad
                                   , int
                                   , bool                                 toAdopt
                                   , XSerializeEngine&                    serEng)
{
    if (serEng.needToLoadObject((void**)objToLoad))
    {
        ...
        {
            *objToLoad = new (serEng.getMemoryManager())
                             RefHashTableOf<MatchAttGroupInfo>(
                                                                hashModulus
                                                              , toAdopt
                                                              , serEng.getMemoryManager()
                                                              );
