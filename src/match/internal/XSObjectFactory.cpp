...
//  Includes
// ---------------------------------------------------------------------------
#include <match/internal/XSObjectFactory.hpp>
#include <match/framework/psvi/XSModel.hpp>
#include <match/framework/psvi/XSParticle.hpp>
#include <match/framework/psvi/XSModelGroup.hpp>
#include <match/framework/psvi/XSElementDeclaration.hpp>
#include <match/framework/psvi/XSComplexTypeDefinition.hpp>
#include <match/framework/psvi/XSSimpleTypeDefinition.hpp>
#include <match/framework/psvi/XSModelGroupDefinition.hpp>
#include <match/framework/psvi/XSAttributeGroupDefinition.hpp>
#include <match/framework/psvi/XSWildcard.hpp>
#include <match/framework/psvi/XSNamespaceItem.hpp>
#include <match/framework/psvi/XSIDCDefinition.hpp>
#include <match/framework/psvi/XSAttributeUse.hpp>
#include <match/framework/psvi/XSAttributeDeclaration.hpp>
#include <match/framework/psvi/XSNotationDeclaration.hpp>
#include <match/framework/psvi/XSFacet.hpp>
#include <match/framework/psvi/XSMultiValueFacet.hpp>
    ...
    , fDeleteVector(0)
{
    ...
    fMatchToXSMap = new (manager) RefHashTableOf<XSObject, PtrHasher> (
        109, false, manager);
}

XSObjectFactory::~XSObjectFactory()
{
    delete fMatchToXSMap;
        ...
        if (typeInfo->hasAttDefs())
        {
            ...
            for(unsigned int i=0; i<attCount; i++)
            {
                ...
                if (attDef.getBaseAttDecl())
                {
                    ...
                    fMatchToXSMap->put(&attDef, xsAttDecl);
                }
                else
...
XSObjectFactory::createXSModelGroupDefinition(MatchGroupInfo* const groupInfo,
                                              XSModel* const xsModel)
{
...
XSObjectFactory::createXSAttGroupDefinition(MatchAttGroupInfo* const attGroupInfo,
                                            XSModel* const xsModel)
{
    XSAttributeUseList* xsAttList = 0;
    XSWildcard*         xsWildcard = 0;
