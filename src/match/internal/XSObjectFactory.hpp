...
#if !defined(MATCHC_INCLUDE_GUARD_XSOBJECTFACTORY_HPP)
#define MATCHC_INCLUDE_GUARD_XSOBJECTFACTORY_HPP

#include <match/framework/psvi/XSConstants.hpp>
...
private:
    ...
    (
        MatchGroupInfo* const groupInfo
        , XSModel* const xsModel
    );

    XSAttributeGroupDefinition* createXSAttGroupDefinition
    (
        MatchAttGroupInfo* const attGroupInfo
        , XSModel* const xsModel
    );
