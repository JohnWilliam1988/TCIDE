...
//  Includes
// ---------------------------------------------------------------------------
#include <match/validators/schema/TraverseSchema.hpp>
#include <match/framework/XMLEntityHandler.hpp>
#include <match/framework/XMLValidityCodes.hpp>
#include <match/validators/schema/identity/IC_Key.hpp>
#include <match/validators/schema/identity/IC_KeyRef.hpp>
#include <match/validators/schema/identity/IC_Unique.hpp>
#include <match/validators/schema/identity/IC_Field.hpp>
#include <match/validators/schema/identity/IC_Selector.hpp>
#include <match/validators/schema/identity/MatchXPath.hpp>
#include <match/util/XMLStringTokenizer.hpp>
#include <match/validators/schema/XUtil.hpp>
#include <match/validators/common/GrammarResolver.hpp>
#include <match/internal/XMLReader.hpp>
#include <match/validators/schema/ComplexTypeInfo.hpp>
#include <match/validators/schema/NamespaceScope.hpp>
#include <match/validators/schema/SchemaAttDefList.hpp>
#include <match/internal/XMLScanner.hpp>
#include <match/framework/LocalFileInputSource.hpp>
#include <match/framework/URLInputSource.hpp>
...
#include <match/validators/schema/GeneralAttributeCheck.hpp>
...
#include <match/validators/schema/XSDDOMParser.hpp>
#include <match/dom/DOMNamedNodeMap.hpp>
#include <match/dom/DOMText.hpp>
#include <match/dom/impl/XSDElementNSImpl.hpp>
#include <match/util/OutOfMemoryException.hpp>
#include <match/util/NumberFormatException.hpp>
#include <match/util/XMLEntityResolver.hpp>
#include <match/util/XMLUri.hpp>
#include <match/framework/psvi/XSAnnotation.hpp>
#include <match/framework/MemBufInputSource.hpp>
#include <match/internal/XSAXMLScanner.hpp>
...
void TraverseSchema::preprocessSchema(DOMElement* const schemaRoot,
    ...
    if (!multipleImport) {
        ...
        if (fGroupRegistry == 0 ) {
            ...
            fGroupRegistry = new (fGrammarPoolMemoryManager) RefHashTableOf<MatchGroupInfo>(13, fGrammarPoolMemoryManager);
        ...
        if (fAttGroupRegistry == 0 ) {
            ...
            fAttGroupRegistry = new (fGrammarPoolMemoryManager) RefHashTableOf<MatchAttGroupInfo>(13, fGrammarPoolMemoryManager);
...
void TraverseSchema::preprocessInclude(const DOMElement* const elem) {
    ...
    fParser->setValidationScheme(MatchDOMParser::Val_Never);
                                       ...
                                       bool& hasChildren)
{
    ...
    for (; child != 0; child = XUtil::getNextSiblingElement(child)) {
        ...
        if (XMLString::equals(childName, SchemaSymbols::fgELT_ELEMENT)) {
            ...
            MatchGroupInfo* grpInfo = traverseGroupDecl(child, false);
...
TraverseSchema::traverseGroupDecl(const DOMElement* const elem,
    ...
    MatchGroupInfo* groupInfo = fGroupRegistry->get(fullName);
    ...
    MatchGroupInfo* saveGroupInfo = fCurrentGroupInfo;

    Janitor<MatchGroupInfo>    newGroupInfoJan(new (fGrammarPoolMemoryManager) MatchGroupInfo(
        fStringPool->addOrFind(name), fTargetNSURI, fGrammarPoolMemoryManager));
    fCurrentGroupStack->addElement(nameIndex);
    MatchGroupInfo* const newGroupInfo = newGroupInfoJan.get();
    ...
    if (fFullConstraintChecking) {
        ...
        if (fRedefineComponents && fRedefineComponents->get(SchemaSymbols::fgELT_GROUP, nameIndex))
        {
            ...
            {
                MatchGroupInfo* baseGroup = fGroupRegistry->get(fBuffer.getRawBuffer());
                if (baseGroup)
                {
...
TraverseSchema::traverseAttributeGroupDecl(const DOMElement* const elem,
    ...
    // Handle "ref="
    // ------------------------------------------------------------------
    MatchAttGroupInfo* attGroupInfo;
    Janitor<MatchAttGroupInfo> janAttGroupInfo(0);
    if (!topLevel) {
    ...
    else
    {
        ...
        MatchAttGroupInfo* saveAttGroupInfo = fCurrentAttGroupInfo;
        janAttGroupInfo.reset(new (fGrammarPoolMemoryManager) MatchAttGroupInfo(
            fStringPool->addOrFind(name), fTargetNSURI, fGrammarPoolMemoryManager));
        ...
        if (fRedefineComponents && fRedefineComponents->get(SchemaSymbols::fgELT_ATTRIBUTEGROUP, nameIndex)) {
            ...
            fBuffer.set(name);
            fBuffer.append(SchemaSymbols::fgRedefIdentifier);
            MatchAttGroupInfo* baseAttGroupInfo = fAttGroupRegistry->get(fBuffer.getRawBuffer());
...
TraverseSchema::traverseAttributeGroupDeclNS(const DOMElement* const elem,
    ...
    MatchAttGroupInfo* attGroupInfo = ((SchemaGrammar*)aGrammar)->getAttGroupInfoRegistry()->get(name);
...
//        reportSchemaError(icElem, XMLUni::fgXMLErrDomain, XMLErrs::IC_BadContent);
    ...
    try {
        ...
        MatchXPath* sXPath = new (fGrammarPoolMemoryManager) MatchXPath(xpathExpr, fStringPool, fSchemaInfo->getNamespaceScope(), fEmptyNamespaceURI, true, fGrammarPoolMemoryManager);
    ...
    while (elem != 0) {
        ...
        if (!XMLString::equals(elem->getLocalName(), SchemaSymbols::fgELT_FIELD)) {
            ...
            try {
                ...
                MatchXPath* fieldXPath = new (fGrammarPoolMemoryManager) MatchXPath
                (
                    xpathExpr
                    , fStringPool
                    , fSchemaInfo->getNamespaceScope()
...
void TraverseSchema::processComplexContent(const DOMElement* const ctElem,
    ...
    if (childElem != 0) {
        ...
        if (XMLString::equals(childName, SchemaSymbols::fgELT_GROUP)) {
            ...
            MatchGroupInfo* grpInfo = traverseGroupDecl(childElem, false);
...
void TraverseSchema::processAttributes(const DOMElement* const elem,
    ...
    MatchAttGroupInfo* attGroupInfo = 0;
    ValueVectorOf<MatchAttGroupInfo*> attGroupList(4, fGrammarPoolMemoryManager);
...
MatchGroupInfo* TraverseSchema::processGroupRef(const DOMElement* const elem,
                                                 const XMLCh* const refName) {
...
void TraverseSchema::processElements(const DOMElement* const elem,
                                     MatchGroupInfo* const fromGroup,
                                     ComplexTypeInfo* const typeInfo)
{
    NamespaceScopeManager nsMgr(elem, fSchemaInfo, this);
...
void TraverseSchema::copyAttGroupAttributes(const DOMElement* const elem,
                                            MatchAttGroupInfo* const fromAttGroup,
                                            MatchAttGroupInfo* const toAttGroup,
                                            ComplexTypeInfo* const typeInfo) {
...
void TraverseSchema::checkAttDerivationOK(const DOMElement* const elem,
                                          const MatchAttGroupInfo* const baseAttGrpInfo,
                                          const MatchAttGroupInfo* const childAttGrpInfo) {
...
void TraverseSchema::validateAnnotations() {
    ...
    grammar->setGroupInfoRegistry(new (memMgr) RefHashTableOf<MatchGroupInfo>(13, memMgr));
    grammar->setAttGroupInfoRegistry(new (memMgr) RefHashTableOf<MatchAttGroupInfo>(13, memMgr));
