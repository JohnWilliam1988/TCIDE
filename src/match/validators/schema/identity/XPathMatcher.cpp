...
//  Includes
// ---------------------------------------------------------------------------
#include <match/validators/schema/identity/XPathMatcher.hpp>
...
// ---------------------------------------------------------------------------
XPathMatcher::XPathMatcher( MatchXPath* const xpath
...
// ---------------------------------------------------------------------------
void XPathMatcher::init(MatchXPath* const xpath) {
...
void XPathMatcher::startElement(const XMLElementDecl& elemDecl,
    ...
    for (XMLSize_t i = 0; i < fLocationPathSize; i++) {
        ...
        while (fCurrentStep[i] < stepSize &&
               locPath->getStep(fCurrentStep[i])->getAxisType() == MatchStep::AxisType_SELF) {
            fCurrentStep[i]++;
        }
        ...
        while (fCurrentStep[i] < stepSize &&
               locPath->getStep(fCurrentStep[i])->getAxisType() == MatchStep::AxisType_DESCENDANT) {
            fCurrentStep[i]++;
        }
        ...
        if ((fCurrentStep[i] == startStep || fCurrentStep[i] > descendantStep) &&
            locPath->getStep(fCurrentStep[i])->getAxisType() == MatchStep::AxisType_CHILD) {

            MatchStep* step = locPath->getStep(fCurrentStep[i]);
            MatchNodeTest* nodeTest = step->getNodeTest();
        ...
        if (fCurrentStep[i] < stepSize &&
            locPath->getStep(fCurrentStep[i])->getAxisType() == MatchStep::AxisType_ATTRIBUTE) {

            if (attrCount) {

                MatchNodeTest* nodeTest = locPath->getStep(fCurrentStep[i])->getNodeTest();
...
bool XPathMatcher::matches(const MatchNodeTest* nodeTest, const QName* qName)
{
    if (nodeTest->getType() == MatchNodeTest::NodeType_QNAME) {
        return (*nodeTest->getName())==(*qName);
    }
    if (nodeTest->getType() == MatchNodeTest::NodeType_NAMESPACE) {
