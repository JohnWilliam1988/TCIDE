...
#if !defined(MATCHC_INCLUDE_GUARD_TRAVERSESCHEMA_HPP)
#define MATCHC_INCLUDE_GUARD_TRAVERSESCHEMA_HPP
...
#include <match/validators/schema/SchemaInfo.hpp>
#include <match/validators/schema/GeneralAttributeCheck.hpp>
#include <match/validators/schema/XSDErrorReporter.hpp>
#include <match/util/XMLResourceIdentifier.hpp>
...
private:
    ...
    ContentSpecNode*    traverseAll(const DOMElement* const allElem,
                                    bool& hasChildren);
    MatchGroupInfo*    traverseGroupDecl(const DOMElement* const childElem,
                                          const bool topLevel = true);
    MatchAttGroupInfo* traverseAttributeGroupDecl(const DOMElement* const elem,
                                                   ComplexTypeInfo* const typeInfo,
                                                   const bool topLevel = false);
    MatchAttGroupInfo* traverseAttributeGroupDeclNS(const DOMElement* const elem,
                                                     const XMLCh* const uriStr,
                                                     const XMLCh* const name);
    ...
    /**
      ...
      */
    MatchGroupInfo* processGroupRef(const DOMElement* const elem,
                                     const XMLCh* const refName);

    /**
      * Process a 'ref' on a attributeGroup
      */
    MatchAttGroupInfo* processAttributeGroupRef(const DOMElement* const elem,
                                                 const XMLCh* const refName,
                                                 ComplexTypeInfo* const typeInfo);
    ...
    RefHashTableOf<MatchGroupInfo>*               fGroupRegistry;
    RefHashTableOf<MatchAttGroupInfo>*            fAttGroupRegistry;
    RefHashTableOf<ElemVector>*                    fIC_ElementsNS;
    RefHashTableOf<SchemaInfo, PtrHasher>*         fPreprocessedNodes;
    SchemaInfo*                                    fSchemaInfo;
    MatchGroupInfo*                               fCurrentGroupInfo;
    MatchAttGroupInfo*                            fCurrentAttGroupInfo;
